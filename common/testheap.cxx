...
public:
  WrapInt() { v = 0; }
  WrapInt(int i) { v = i; }
  bool operator<(const WrapInt& w) const { return this->v < w.v; }
  const WrapInt& operator=(const WrapInt& w) { this->v=w.v; return *this;}
  int v;
};
...
int main()
{
  ...
  for (unsigned int i = 0; i < origSize; i++)
    std::cout << heap.ExtractMinimum().v << " X ";
  std::cout << std::endl;
  ...
  for (unsigned int i = 0; i < origSize; i++)
    std::cout << heapf.ExtractMinimum() << " X ";
  std::cout << std::endl;
